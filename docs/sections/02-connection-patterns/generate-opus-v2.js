// 第2章スライド v2: 現行版(generate-opus.js)を保持したまま、レビュー指摘の3点を反映した別バージョン。
//  1. 02-01 に用語補足（OPC UA / エッジ）を追加
//  2. エッジ経由のタイトルを実態に合わせて「（中心は IoT Operations）」に修正
//  3. 情報過多だった 02-03 を「IoT Operations」と「IoT Edge」の2枚に分割し、割り切りの一文を明示
// 出力は 02-connection-patterns-opus-v2.pptx（現行版は上書きしない）。
...
pptx.title = "第2章 接続パターンとサービスの選択（v2）";
...
function footer(slide, n) {
  ...
  slide.addText(RUNNING, { x: MX, y: 7.06, w: 9.5, h: 0.3, fontFace: JP, fontSize: 9, color: P.muted, margin: 0, valign: "middle" });
  slide.addText(String(n), { x: RIGHT - 0.6, y: 7.06, w: 0.6, h: 0.3, fontFace: JP, fontSize: 10, color: P.muted, align: "right", margin: 0, valign: "middle" });
}
...
function note(slide, text, y) {
  slide.addText(text, { x: MX, y, w: CW, h: 0.26, fontFace: JP, fontSize: 11, color: P.muted, margin: 0, valign: "middle" });
}
...
function tableAt(slide, head, rows, y, opts = {}) {
  ...
  slide.addTable(body, { x, y, w, colW, rowH: opts.rowH || 0.42, border: { pt: 0.75, color: P.line }, margin: [3, 6, 3, 6], valign: "middle", autoPage: false });
}
...
function cover() {
  ...
  s.addText("クラウド直接接続とエッジ経由接続を要件から選び、今回は IoT Hub への直接接続を扱う", { x: 0.9, y: 3.25, w: 11.5, h: 0.5, fontFace: JP, fontSize: 17, color: "BFE6EC", margin: 0 });
  ...
  s.addText("Azure IoT ワークショップ ／ 本編 8 分・5 枚", { x: 0.9, y: 6.5, w: 11, h: 0.4, fontFace: JP, fontSize: 12, color: "9FC7D1", margin: 0 });
}

// ---- 02-01 接続パターンを決める3つの要件（用語補足つき） ---------------
function s01() {
  ...
  ], 2.35, { x: MX, w: 6.9, colW: [1.7, 5.2], rowH: [0.5, 1.06, 0.78, 0.78], size: 11.5, hsize: 12 });
  ...
  nodeBox(s, bx, 3.82, bw, 0.92, "クラウド直接接続", "Azure IoT Hub", { fill: P.softer, titleColor: P.primary, border: P.line, tSize: 13.5, sSize: 12, subColor: P.teal });
  nodeBox(s, bx, 4.86, bw, 1.08, "エッジ経由接続", "Azure IoT Operations\nAzure IoT Edge", { fill: P.softer, titleColor: P.primary, border: P.line, tSize: 13.5, sSize: 12, subColor: P.teal });
  note(s, "エッジ = デバイスや設備に近い現場側の実行環境", 6.06);
...
// ---- 02-03 エッジ経由接続（中心は IoT Operations） ----------------------
function s03a() {
  const s = pptx.addSlide(); header(s, "03", "エッジ経由接続", "エッジ経由接続（中心は IoT Operations）");
  lead(s, "IoT Operations の構成例: 設備のデータを現場で収集・加工し、クラウドへ送る。");
  note(s, "この章は「エッジ経由という道がある」ことを押さえれば十分。個々の部品名は覚えなくてよい。", 2.04);
  ...
  s.addText("工場・現場 ／ Azure IoT Operations（Azure Arc 対応 Kubernetes）", { x: boxX, y: boxY + 0.03, w: rtW, h: 0.24, fontFace: JP, fontSize: 10, bold: true, color: P.teal, align: "center", margin: 0 });
  ...
  const py = 3.98, ph = 1.02, pw = (CW - 0.5) / 3;
  ...
  callout(s, "IoT Operations の注意: オフライン動作は最大72時間で、機能が低下する可能性がある。完全閉域での無期限運用を前提にしない。", 5.24);
  sources(s, [
...
// ---- 02-04 エッジ経由のもう一つの選択肢: Azure IoT Edge ----------------
function s03b() {
  const s = pptx.addSlide(); header(s, "04", "エッジ経由の選択肢", "エッジ経由のもう一つの選択肢: Azure IoT Edge");
  lead(s, "エッジ経由には、IoT Operations のほかに Azure IoT Edge という選択肢もある。");

  // ゲートウェイ構成の小さな図
  const y = 2.5, h = 1.0;
  const dvw = 2.5, edw = 3.4, hbw = 2.5;
  const gap = (CW - dvw - edw - hbw) / 2;
  const dvx = MX, edx = MX + dvw + gap, hbx = MX + dvw + gap + edw + gap;
  nodeBox(s, dvx, y, dvw, h, "下位デバイス", "現場のセンサー・機器", { fill: P.softer, titleColor: P.primary, tSize: 13, sSize: 10.5 });
  nodeBox(s, edx, y, edw, h, "Azure IoT Edge", "コンテナー実行 ／ ゲートウェイ", { fill: P.soft, titleColor: P.primary, border: P.amber, borderW: 1.5, tSize: 15, sSize: 11, subColor: P.amber });
  nodeBox(s, hbx, y, hbw, h, "Azure IoT Hub", "クラウド側の接続先", { fill: P.softer, titleColor: P.primary, tSize: 13, sSize: 10.5 });
  arrow(s, dvx + dvw + 0.05, y + h / 2, gap - 0.1, "メッセージ");
  arrow(s, edx + edw + 0.05, y + h / 2, gap - 0.1, "中継して接続");

  const cy = 3.95, ch = 1.5, cw = (CW - 0.5) / 3;
  pointCard(s, MX, cy, cw, ch, "コンテナー実行", P.cyan, "デバイス上でコンテナー化した処理（分析・フィルタリングなど）を現場で実行する。");
  pointCard(s, MX + cw + 0.25, cy, cw, ch, "IoT Hub へのゲートウェイ", P.teal, "下位デバイスのメッセージを中継し、IoT Hub へ接続する経路として使える。");
  pointCard(s, MX + (cw + 0.25) * 2, cy, cw, ch, "現状と本章での扱い", P.amber, "現在もサポートされている（2026-09-08 確認）。本章では紹介に留め、構築手順や詳細比較は扱わない。");

  callout(s, "使い分け: 産業プロトコル変換や現場処理が中心なら IoT Operations、デバイス上のコンテナー実行と IoT Hub 連携なら IoT Edge を検討する。", 5.66);
  sources(s, [
    "https://learn.microsoft.com/ja-jp/azure/iot-edge/about-iot-edge",
    "https://learn.microsoft.com/ja-jp/azure/iot-edge/iot-edge-as-gateway",
    "https://learn.microsoft.com/ja-jp/azure/iot-edge/version-history",
  ]);
  footer(s, 5);
}

// ---- 02-05 ハイブリッド構成と今回の対象範囲 -----------------------------
function s04() {
  const s = pptx.addSlide(); header(s, "05", "使い分けと対象範囲", "ハイブリッド構成と今回の対象範囲");
...
cover();
s01(); s02(); s03a(); s03b(); s04();

pptx.writeFile({ fileName: "docs/sections/02-connection-patterns/02-connection-patterns-opus-v2.pptx" }).then((f) => console.log("written:", f));
