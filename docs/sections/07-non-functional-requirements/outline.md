...
非機能要件を説明した後、本番移行判定の前に、複数のサービスを組み合わせた構成図を使って設計判断を振り返る。新しいサービスを網羅的に解説するのではなく、今回学んだ IoT Hub の役割と、その外側で必要になる処理・運用を具体化する。

![RNA のケーススタディ構成図。ブランド別の IoT Hub、Service Bus の High／Normal、後続処理と外部システムへの連携を示す。](case-study-rna.png)

出典: [IoT Readiness Refresher 2025 - Day 01](../../references/IoT%20Readiness%20Refresher%202025%20-%20Day%2001.pptx)、スライド11「Case Study From RNA」。元資料の埋め込み画像を無改変で掲載している。

**この図は具体的な事例を参考に設計を考えるためのものであり、すべての IoT システムに共通する標準構成や、現在の公式推奨構成として示すものではない。** 固有名詞・サービス表記は原図のままとし、採用理由や現在の稼働状況は図だけから断定しない。資料を外部へ配布する際は、元資料と図の利用・公開条件を確認する。

### 図で見る3つの設計判断

次の「問い」は、本ワークショップで事例を読み解くための設計観点であり、事例の実装仕様を断定するものではない。

| 観点 | 原図に描かれていること | 受講者に投げかける問い |
| --- | --- | --- |
| 接続先の分離 | Protocol Gateway の後に Nissan 用と Renault 用の IoT Hub がある | 自分のシステムでは、顧客・ブランド・拠点など、どの単位で接続先や運用を分ける必要があるか |
| 優先度別の処理 | Service Bus に High／Normal の経路があり、Telemetry processor につながる | 急ぐメッセージと通常データをどう分け、混雑時にも必要な処理時間を満たすか。名称だけで優先処理の保証があると考えない |
| 後続サービスとの責任分界 | IoT Hub の後段に処理、Extensions、External Systems への連携がある | どこまで到達したら業務処理完了とするか。途中で止まった場合、誰が検知・再処理・復旧を担うか |

図を読む順序は、車両 → Protocol Gateway → ブランド別 IoT Hub → Service Bus → Telemetry processor → Extensions → External Systems とする。上段のサービス有効化に関する処理や、下段の個別連携経路は概要紹介に留める。図だけでは分からない認証・ネットワーク制御、冗長化、再試行、重複対策、監視を、本章の要件と照合する。

座学では約3分で説明する。スライドでは元図を大きく配置し、上記3箇所を順に強調する。出典と「事例構成。共通の必須構成ではない」の注記を付け、詳細な問いは発表者ノートに置く。公式の構成例や本番化の検討項目へ進むための補助資料として、Azure Architecture Center を案内する（[IoT アーキテクチャの設計](https://learn.microsoft.com/ja-jp/azure/architecture/guide/iot/iot-get-started)、[IoT Hub ベースのソリューションの本番環境への移行](https://learn.microsoft.com/ja-jp/azure/architecture/example-scenario/iot/iot-move-to-production)）。
...
6. ケーススタディの構成図で、接続先の分離・優先度別の処理・後続サービスとの責任分界を読み解く
7. 図だけでは確認できない非機能要件を挙げ、本番移行判定チェックリストにつなげる
...
| 8 | ケーススタディで本番構成を読み解く | 学んだ非機能要件を具体的な構成に当てはめる | RNA の事例図を大きく掲載し、接続先の分離・優先度別の処理・責任分界を順に強調 | 構成をそのまま採用せず、自分の要件に照らして設計判断を読み解く |
| 9 | 本番移行判定チェックリスト | 最後の確認観点を示す | 重要度付きチェックリスト | 必須項目が未定義なら、本番移行のリスクが高い |
...
- RNA のケーススタディ構成図（Day 01、スライド11）。事例固有の構成と、自分の要件に基づく設計判断を区別する
- 本番移行判定チェックリスト
...
- ケーススタディの構成をそのまま採用すれば、自分のシステムの非機能要件も満たせる
...
- ケーススタディの図だけでは確認できない本番要件は何か。自分のシステムでは、接続先の分離と業務処理完了の範囲をどう決めるか
