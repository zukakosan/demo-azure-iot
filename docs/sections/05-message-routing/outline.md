...
| 5 | 配送経路の明示と到達確認 | 通常ルートとフォールバックを区別し、各宛先で検証する設計方針を示す | Storage と events への明示ルートによる同時配送と、条件不一致時だけのフォールバックを対比 | 必要な経路は組み込みも含めて明示し、送信成功だけでなく各宛先で到達確認する。配送先の障害は別途監視する |
